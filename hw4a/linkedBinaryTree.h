...
class linkedBinaryTree : public binaryTree<binaryTreeNode<E> >
{
   public:
    ...
    {
        public:
            vector<binaryTreeNode<E>*> visited;
            stack<binaryTreeNode<E>*> nodeQueue;

            inorderIterator() {
            }

            inorderIterator (binaryTreeNode<E> * newRoot) {
            ...
            binaryTreeNode<E>* next() {
                while (nodeQueue.size() > 0) {
                    binaryTreeNode<E> * first = nodeQueue.top();
                    nodeQueue.pop();

                    while (first->leftChild != NULL && !hasVisited(first->leftChild) ) {
                        nodeQueue.push(first);
                        first = first->leftChild;    
                    }

                    if (first->rightChild != NULL && !hasVisited(first->rightChild) ) {
                        nodeQueue.push(first->rightChild);
                    }
            ...
            bool hasVisited(binaryTreeNode<E> * node) {
                typename vector<binaryTreeNode<E>*>::iterator vect;
                for (vect = visited.begin(); vect < visited.end(); vect++){
                    if (*vect == node){
                        return true;
                    }
                }
      ...
      class postorderIterator
    {
        public:
            ...
            postorderIterator(binaryTreeNode<E> * theRoot)
            {
                nodeQueue.push(theRoot);
            }
    ...
    postorderIterator* beginPO () {
          postorderIterator *i = new postorderIterator(root);
          return i;
      }
...
{// Remove and return the left subtree.
   ...
   return  (linkedBinaryTree<E>&) *leftSubtree;
}
